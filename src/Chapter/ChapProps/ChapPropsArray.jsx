    ...
    return (
        <div>
            ...
            <ul>
                ...
                {/*
                 1. map() -> return 생략 가능 / UI 로 표현해줄 때 많이 사용
                 Parent 에서 가져온 변수이름.map((반복문 내에서 사용할 변수이름, 번호순서 = index) =>( <li key={index}> {user.name} / {user.age}       </li>)             */}
                {/*
                2. map{}  -> return 반드시 사용해야함 / UI 와 기능을 동시에 표현할 때 많이 사용
                Parent 에서 가져온 변수이름.map((반복문 내에서 사용할 변수이름, 번호순서 = index) =>{ return( <li key={index}> {user.name} / {user.age}       </li>  );   }  )          */}

                {/*
                        html 태그 내에서 javascript로 가져온 값을 사용하기 위해서는
                        반드시 {} 를 작성 후 변수이름이나 기능 작성
                 */}
                {
...
const Parent = () => {
    ...
    // 만약 배열에서 한 칸 당 값을 두개씩 가져오고 싶다면 {}로 감싸서 가져오기
    ...
    // [{ React 에서 가져온 DB를 사용할 변수이름1번  : Java Backend 에서 전달받은 데이터 1번        ,
    //    React 에서 가져온 DB를 사용할 변수이름2번  : Java Backend 에서 전달받은 데이터 2번      }]
    ...
    return (
        <>
            ...
            {/*  ChildThree 에서 사용할 변수 명칭 = users  사용할 데이터 = userList에 작성한 데이터를 사용  */}
